                        ...
                        <table class="table border w-full text-sm">
                            <thead>
                                <tr>
                                    ...
                                    <th class="p-2 border text-center">Detail</th>
...
@foreach($items as $item)
                                <tr>
                                    ...
                                    <td class="p-2 border text-center">{{ $item->detail }}</td>
...
@endif
    ...
    <x-modal name="create-menu-item" :show="$errors->paketCreate->isNotEmpty()" focusable>
        <form method="POST" action="{{ route('admin.paket.store')}}" class="p-6" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mt-6">
                ...
                />
                <x-input-error :messages="$errors->paketCreate->get('name')" class="mt-2" />
            </div>
            <div class="mt-6">
                ...
                <x-text-input
                    ...
                    placeholder="Price"
                />
                <x-input-error :messages="$errors->paketCreate->get('price')" class="mt-2" />
            </div>
            <div class="mt-6">
                <x-input-label for="detail" value="{{ __('detail') }}" /><span>Seperate with (-) min</span>
                <x-text-input
                    id="detail"
                    name="detail"
                    type="text"
                ...
                />
                <x-input-error :messages="$errors->paketCreate->get('detail')" class="mt-2" />
            </div>
    ...
    <x-modal name="update-menu-item" :show="$errors->PaketUpdate->isNotEmpty()" focusable>
        <form method="POST" action="{{ route('admin.paket.update')}}" class="p-6" enctype="multipart/form-data">
            @csrf
            ...
            <div class="mt-6">
                ...
                <x-text-input
                    ...
                    placeholder="Menu Name"
                />
                <x-input-error :messages="$errors->PaketUpdate->get('name')" class="mt-2" />
            </div>
            <div class="mt-6">
                <x-input-label for="price" value="{{ __('Price') }}" />
                <x-text-input
                    ...
                    placeholder="Price"
                />
                <x-input-error :messages="$errors->PaketUpdate->get('price')" class="mt-2" />
            </div>
            <div class="mt-6">
                <x-input-label for="detail" value="{{ __('detail') }}" /><span>Seperate with (-) min</span>
                <x-text-input
    ...
    <x-modal name="delete-menu" :show="$errors->PaketDelete->isNotEmpty()" focusable>
        <form method="POST" action="{{ route('admin.paket.destroy')}}" class="p-6">
            @csrf
